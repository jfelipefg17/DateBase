...
SELECT NOMBRE, CARGO_EMP FROM EMPLEADOS WHERE CARGO_EMP LIKE 'Jefe%';
...
SELECT SAL_EMP AS Salario, COMISION_EMP AS Comision, (SAL_EMP + COMISION_EMP) AS SalarioTotal FROM EMPLEADOS WHERE COMISION_EMP > 1000; 

SELECT SAL_EMP AS Salario, COMISION_EMP AS Comision, (SAL_EMP + COMISION_EMP) AS SalarioTotal FROM EMPLEADOS WHERE COMISION_EMP = 0;
...
SELECT * FROM DEPARTAMENTOS WHERE NOMBRE_DEPTO = 'VENTAS' OR NOMBRE_DEPTO = 'MANTENIMIENTO' OR  NOMBRE_DEPTO ='INVESTIGACION';
...
SELECT MAX(SAL_EMP) AS ALTO, MIN(SAL_EMP) AS BAJO, MAX(SAL_EMP) - MIN(SAL_EMP) AS DIFERENCIA FROM EMPLEADOS;

SELECT ID_DEPTO AS NOMBRE, AVG(SAL_EMP) AS PROMEDIO FROM EMPLEADOS GROUP BY ID_DEPTO ORDER BY ID_DEPTO;

SELECT ID_DEPTO AS ID, COUNT(*) AS NUMEROEMPLEADOS FROM EMPLEADOS GROUP BY ID_DEPTO HAVING COUNT(*) >=3;
 
SELECT ID_DEPTO AS ID, COUNT(*) AS NUMEROEMPLEADOS FROM EMPLEADOS GROUP BY ID_DEPTO HAVING COUNT(*) = 0;
 
SELECT * FROM EMPLEADOS WHERE SAL_EMP > (AVG(SAL_EMP) * 0.3) GROUP BY ID_DEPTO ORDER BY ID_DEPTO;
